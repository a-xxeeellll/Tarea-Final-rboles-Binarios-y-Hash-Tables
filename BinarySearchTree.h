...
public:
	...
	// ***** PUNTO 2: Implementar "Search" de manera iterativa (agregado) *****
	bool Search(T value) const
	{
		...
		while (cur != nullptr)
		{
			...
			cur = (value < cur->value) ? cur->leftChild : cur->rightChild;
		}
		return false;
	}
	// ***** FIN PUNTO 2 *****
	...
	// ***** PUNTO 4: Post-order iterativo (agregado) *****
	void PostOrderIterativo()
	{
		...
		while (cur || !st.empty())
		{
			...
			{
				Node* top = st.back();
				if (top->rightChild && last != top->rightChild)
				{
					cur = top->rightChild;
				}
				else
	...
	// ***** PUNTO 3: Función que viaja izq, viaja der y borra el nodo (agregado) *****
	void ClearPostOrder()            // recursiva (20 pts)
	{
		ClearPostOrderImpl(root);
		root = nullptr;
	}

	void ClearPostOrderIterative()   // iterativa (30 pts)
	{
		if (!root) return;
		...
		while (cur || !st.empty())
		{
			...
			else
			{
				...
				else
				{
					st.pop_back();
					last = top;
					delete top; // borra el nodo actual
				}
			}
		}
		root = nullptr;
...
private:
	...
	*/
	...
	// Helper recursivo para ClearPostOrder (agregado)
	void ClearPostOrderImpl(Node* n)
	{
		if (!n) return;
		ClearPostOrderImpl(n->leftChild);
		ClearPostOrderImpl(n->rightChild);
		delete n;
	}
